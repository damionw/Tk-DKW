...
sub Populate
   {
    ...
    $this->ConfigSpecs
       (
        ...
        '-font' => ['METHOD', 'font', 'Font', '-*-Helvetica-Medium-R-Normal--*-120-*-*-*-*-*-*'],
...
sub font
   {
    ...
    if (defined ($p_Font))
       {
        $this->{'Configure'}{'-font'} = $p_Font;

        foreach my $l_Frame ($this->children())
           {
            foreach my $l_Label ($l_Frame->children())
               {
                next unless $l_Label->isa ('Tk::Label');
                $l_Label->configure ('-font' => $p_Font);
               }
           }
       }

    return $this->{'Configure'}{'-font'};
   }
...
sub MenuLabel
   {
    ...
    $l_Label->afterIdle
       (
        sub {$l_Label->configure ('-font' => $this->cget ('-font'));}
       );
